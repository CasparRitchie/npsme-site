// src/pages/EnvolaQuestionDetail.jsx
...
function AnswerTable({ rows, highlightQids = [], compact = false }) {
  ...
  const grouped = useMemo(() => {
    ...
    // de-dupe values within each group (multi-select can repeat)
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-white/10">
          {grouped.map((g, idx) => {
            const isHot = highlight.has(String(g.question_id));

            return (
              <tr
                key={`${g.question_id}-${idx}`}
                className={isHot ? "bg-indigo-500/10" : ""}
              >
                <td className="py-3 pr-4 align-top">
                  <div className="flex items-start gap-2">
                    {g.question_id ? (
                      <span className="mt-0.5 rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-[11px] text-slate-300">
                        {g.question_id}
                      </span>
                    ) : null}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {g.values.map((v) => (
                      <span
                        ...
                        className={
                          isHot
                            ? "rounded-xl border border-indigo-500/25 bg-indigo-500/10 px-3 py-2 text-slate-100"
                            : "rounded-xl border border-white/10 bg-black/10 px-3 py-2 text-slate-200"
                        }
                      >
                        {v}
                      </span>
...
export default function EnvolaQuestionDetail() {
  ...
  const extractRowForQuestion = (resp, targetQid, followupsMap) => {
    ...
    return {
      ...
      isScoredQuestion,
      rawAnswers,
      rawSelectedOptions: Array.isArray(resp?.selected_options) ? resp.selected_options : [],
    };
  };
  ...
  // Raw answers view controls
  const [rawView, setRawView] = useState("relevant"); // "relevant" | "all"
  const [rawShowJson, setRawShowJson] = useState(false);

  const rawRows = useMemo(() => {
    // Don’t render hundreds of huge tables at once.
    // Default the raw panel to first 30 responses.
    return (state.rows || []).slice(0, 30);
  }, [state.rows]);
                ...
                <>
                  ...
                  <div className="mt-6 rounded-2xl border border-white/10 bg-black/10 p-5">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div className="text-sm font-semibold text-white">
                          {tr("envola.qd.raw", "Raw answers")}
                        </div>
                        <div className="mt-1 text-xs text-slate-400">
                          {tr(
                            "envola.qd.rawSub",
                            "A readable view of the full survey response (questions + answers)."
                          )}
                        </div>
                      </div>

                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-xs text-slate-400">{tr("common.view", "View")}:</span>
                        ...
                        >
                          {tr("envola.qd.relevantOnly", "Relevant only")}
                        </button>
                        ...
                        >
                          {tr("envola.qd.allQuestions", "All questions")}
                        </button>
                        ...
                        <label className="ml-2 inline-flex items-center gap-2 text-xs text-slate-300">
                          ...
                          />
                          {tr("envola.qd.showJson", "Show JSON")}
                        </label>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 grid gap-3">
                      {rawRows.map((r) => {
                        const submitted = prettyDate(r.submitted_at);
                        const score = r.score_0_10;
                        const bucket = r.bucket;

                        const selected = Array.from(
                          new Set([...(r.rawSelectedOptions || []), ...(r.optionAnswers || [])].filter(Boolean))
                        );

                        const highlightQids = [String(questionId), ...followupIds];

                        const allAnswers = Array.isArray(r.rawAnswers) ? r.rawAnswers : [];
                        const relevantAnswers = allAnswers.filter((a) => {
                          const qid = a?.question_id != null ? String(a.question_id) : "";
                          return highlightQids.includes(qid);
                        });
                        ...
                        return (
                          <Disclosure
                            key={r.response_id || submitted}
                            title={tr("envola.qd.response", "Survey response")}
                            right={submitted}
                          >
                            <div className="flex flex-wrap items-center gap-2">
                              {score != null ? <Chip tone="indigo">Score: {score}/10</Chip> : null}

                              {bucket ? (
                                <Chip
                                  tone={
                                    String(bucket).toLowerCase().includes("prom")
                                      ? "green"
                                      : String(bucket).toLowerCase().includes("det")
                                      ? "red"
                                      : "amber"
                                  }
                                >
                              ...
                              {r.response_id ? (
                                <Chip title="Intercom response_id">Receipt: {r.response_id}</Chip>
                              ) : null}

                              {r.contact_id ? (
                                <IntercomContactPill id={r.contact_id} url={r.intercom_contact_url} />
                              ) : null}
                            </div>
                            ...
                            {selected.length ? (
                              <div className="mt-4">
                                <div className="text-xs text-slate-400">
                                  {tr("envola.qd.selectedOptions", "Selected options")}
                                </div>
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {selected.map((opt) => (
                                    <Chip key={opt} tone="neutral" title={opt}>
                                      {opt}
                                    </Chip>
                                  ))}
                                </div>
                            ...
                            <div className="mt-4 rounded-2xl border border-white/10 bg-black/10 p-4">
                              <div className="flex items-center justify-between gap-3">
                                <div className="text-xs font-semibold text-slate-200">
                                  {rawView === "all"
                                    ? tr("envola.qd.allAnswers", "All answers")
                                    : tr("envola.qd.relevantAnswers", "This question + inferred follow-ups")}
                                </div>
                                <div className="text-xs text-slate-400">
                                  {tr("common.count", "Count")}: {answersToShow.length}
                                </div>
                              </div>

                              {answersToShow.length ? (
                                <div className="mt-3">
                                  <AnswerTable rows={answersToShow} highlightQids={highlightQids} compact={false} />
                                </div>
                              ) : (
                                <div className="mt-3 text-sm text-slate-300">
                                  {tr("envola.qd.noRaw", "No raw answers found for this response (unexpected).")}
                                </div>
                              )}
                            </div>

                            {rawShowJson ? (
                              <pre className="mt-4 max-h-80 overflow-auto rounded-2xl border border-white/10 bg-black/20 p-4 text-[11px] text-slate-200">
                                {JSON.stringify(
                                  {
                    ...
                    <div className="mt-4 text-xs text-slate-500">
                      {tr(
                        "envola.qd.rawNote",
                        "Showing up to the first 30 matching responses to keep the page fast. Use the window/sample controls above to change what’s included."
                      )}
                    </div>
                  </div>
