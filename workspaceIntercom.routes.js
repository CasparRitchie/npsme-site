...
router.get("/invitations/reconcile", async (req, res) => {
  ...
  router.get("/sources/active/responses", async (req, res) => {
    try {
      ...
      console.error(
        "[workspace-intercom] GET /sources/active/responses error",
        err
      );
      ...
      return res.status(500).json({
        ...
        error:
          err.message ||
          "Failed to load active source responses",
      });
    }
  });
...
async function buildWorkspaceIntercomPerformancePayload({
  ...
  if (!contentId) {
    throw new Error(
      "Active source is missing survey_content_id"
    );
  }
  ...
  const granularity = [
    ...
    "week",
    "month",
  ].includes(requestedGranularity)
    ? requestedGranularity
    : choosePerformanceGranularity(window);

  const canonicalRows =
    await getCanonicalResponses();

  /*
   * Prepare the source population once so that the current
   * and previous periods use exactly the same data rules.
   */
  const preparedRows = (canonicalRows || [])
    .filter(
      (row) =>
    ...
    .map((row) => ({
      ...row,
      _submitted_ms: Date.parse(
        row?.submitted_at || ""
      ),
      _score: normaliseNpsScore(
        row?.score_0_10
      ),
    }))
    .filter((row) =>
      Number.isFinite(row._submitted_ms)
    );
  ...
  const rows = preparedRows
    ...
    .sort(
      (a, b) =>
        b._submitted_ms - a._submitted_ms
    );

  const previousRows = preparedRows.filter(
    (row) =>
      row._submitted_ms >=
        window.previousFromMs &&
      row._submitted_ms <=
        window.previousToMs
  );
  ...
  const previousSummary =
    buildPerformanceSummary(previousRows);
  ...
  return {
    ...
    period: {
      ...
      previous_from: toYmdUtc(
        window.previousFromMs
      ),
      previous_to: toYmdUtc(
        window.previousToMs
      ),
    },
...
function buildPerformanceSummary(rows) {
  ...
  const scoredRows = safeRows.filter(
    (row) =>
      normaliseNpsScore(row?._score) !== null
  );
  ...
  const promoters = scoredRows.filter(
    (row) =>
      scoreBucket(row._score) === "promoter"
  ).length;

  const passives = scoredRows.filter(
    (row) =>
      scoreBucket(row._score) === "passive"
  ).length;

  const detractors = scoredRows.filter(
    (row) =>
      scoreBucket(row._score) === "detractor"
  ).length;

  const scoreTotal = scoredRows.reduce(
    (sum, row) => sum + row._score,
    0
  );
...
function buildPerformanceComparison({
  ...
  return {
    ...
    nps_change: difference(
      current?.nps,
      previous?.nps
    ),

    average_score_change: difference(
      current?.averageScore,
      previous?.averageScore
    ),

    response_count_change:
      Number(current?.validResponses || 0) -
      Number(previous?.validResponses || 0),
  };
}

function buildPerformanceTimeseries(
  rows,
  granularity,
  window
) {
...
function buildPerformanceBenefits(rows) {
  ...
  return Array.from(byBenefit.values())
    .map((item) => ({
      ...
      promoter_mentions:
        item.promoter_mentions,

      passive_mentions:
        item.passive_mentions,
