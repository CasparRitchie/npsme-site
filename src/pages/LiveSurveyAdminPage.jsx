...
// function computeNps(scores) {
//   const clean = (scores || []).filter((s) => typeof s === "number" && !Number.isNaN(s));
...
// // Generic circular dial (0–100)
// function PercentDial({ label, value, sublabel }) {
//   const size = 92;
//   const stroke = 10;
...
//   return (
//     <div className="rounded-3xl border border-slate-800 bg-slate-950/60 p-4 shadow-xl shadow-black/40">
//       <div className="flex items-center gap-4">
//         <svg width={size} height={size} className="shrink-0">
//           <circle cx={size/2} cy={size/2} r={r} strokeWidth={stroke} fill="none" className="stroke-slate-800" />
//           <circle
//             cx={size/2}
//             cy={size/2}
//             r={r}
...
//         </svg>
//         <div>
//           <p className="text-xs tracking-widest text-slate-400 uppercase">{label}</p>
//           <p className="text-2xl font-semibold text-slate-50">{v.toFixed(0)}%</p>
...
// // NPS dial (-100..+100)
// function NpsDial({ label, value, sublabel }) {
//   const size = 92;
//   const stroke = 10;
//   const r = (size - stroke) / 2;
//   const c = 2 * Math.PI * r;
...
//         <div>
//           <p className="text-xs tracking-widest text-slate-400 uppercase">{label}</p>
//           <p className="text-2xl font-semibold text-slate-50">{isNull ? "—" : v}</p>
//           {sublabel ? <p className="text-xs text-slate-400 mt-1">{sublabel}</p> : null}
//         </div>
//       </div>
//     </div>
...
//         <div className="text-sm font-semibold text-slate-100">{pct}%</div>
//       </div>
//     </div>
//   );
// }
...
//   const description = "Review and send live NPS invitations from your Envola customer list.";
...
//       if (!invRes.ok) throw new Error(invData.error || "Failed to load invitations");
//       if (!respRes.ok) throw new Error(respData.error || "Failed to load responses");
...
//       if (!res.ok || !data.ok) throw new Error(data.error || "Batch send failed");

//       const successes = (data.results || []).filter((r) => r.ok).length;
//       const failures = (data.results || []).filter((r) => !r.ok).length;

//       setStatusMsg(
//         `Sent ${successes} invitation${successes === 1 ? "" : "s"}` +
...
//       setError(e.message || "We couldn’t send those invitations.");
//     } finally {
//       setSending(false);
//     }
//   }
...
//       setStatusMsg(`Resent invitation ${invitationId}.`);
//       await loadAll();
//     } catch (e) {
//       console.error("resend error", e);
//       setError(e.message || "We couldn’t resend that invitation.");
//     } finally {
//       setResendingId("");
//     }
//   }
...
//       <Seo path="/live-survey-admin-page" title={title} description={description} />

//       <main className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12 space-y-6">
//         <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
//           <div>
//             <p className="text-xs tracking-widest text-slate-400 uppercase">Live programme · Envola</p>
//             <h1 className="text-2xl sm:text-3xl font-semibold text-slate-50">Live survey admin</h1>
...
//           <div className="flex items-center gap-3">
//             <ResponseDial value={responseRate} />
//             <button
//               type="button"
//               onClick={loadAll}
//               disabled={loading}
...
//           <KpiCard label="Total" value={total} sub="All invitations in file" />
//           <KpiCard label="Pending" value={pending.length} sub="Not yet sent" />
//           <KpiCard label="Sent" value={sent.length} sub="Delivered or queued" />
//           <KpiCard label="Started" value={started.length} sub="Opened survey link" />
//           <KpiCard
//             label="Completed"
//             value={completed.length}
//             sub={nps === null ? "NPS: n/a" : `NPS: ${nps}`}
//           />
//         </section>
...
//             <div>
//               <h2 className="text-lg font-semibold text-slate-50">Pending</h2>
//               <p className="text-xs text-slate-400">Select and send invitations.</p>
//             </div>
...
//         <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 sm:p-6 shadow-xl shadow-black/40">
//           <div className="mb-4">
//             <h2 className="text-lg font-semibold text-slate-50">Sent</h2>
//             <p className="text-xs text-slate-400">Resend if someone cannot find the email.</p>
//           </div>

//           {sent.length === 0 ? (
//             <p className="text-sm text-slate-400">No sent invitations.</p>
//           ) : (
//             <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/60">
//               <table className="min-w-full text-xs">
//                 <thead className="bg-slate-900/70">
...
//                     <th className="px-3 py-2 text-left text-slate-300">Name</th>
//                     <th className="px-3 py-2 text-left text-slate-300">Email</th>
//                     <th className="px-3 py-2 text-left text-slate-300">Last sent</th>
//                     <th className="px-3 py-2 text-left text-slate-300">Resends</th>
...
//                         <td className="px-3 py-2">{row.customerName}</td>
//                         <td className="px-3 py-2 text-slate-300">{row.email}</td>
//                         <td className="px-3 py-2 text-slate-300">{row.lastSentAt || row.sentAt || "-"}</td>
//                         <td className="px-3 py-2 text-slate-300">{row.resentCount || 0}</td>
//                         <td className="px-3 py-2">
//                           <button
//                             type="button"
...
//         <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 sm:p-6 shadow-xl shadow-black/40">
//           <div className="mb-4">
//             <h2 className="text-lg font-semibold text-slate-50">Started</h2>
//             <p className="text-xs text-slate-400">These recipients opened the survey link.</p>
//           </div>

//           {started.length === 0 ? (
//             <p className="text-sm text-slate-400">No started invitations.</p>
//           ) : (
//             <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/60">
//               <table className="min-w-full text-xs">
//                 <thead className="bg-slate-900/70">
...
//         <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 sm:p-6 shadow-xl shadow-black/40">
//           <div className="mb-4">
//             <h2 className="text-lg font-semibold text-slate-50">Completed</h2>
//             <p className="text-xs text-slate-400">Scores are shown when a response exists in /npsme/live/responses.csv.</p>
//           </div>

//           {completed.length === 0 ? (
//             <p className="text-sm text-slate-400">No completed invitations.</p>
//           ) : (
//             <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/60">
//               <table className="min-w-full text-xs">
//                 <thead className="bg-slate-900/70">
...
//                     <th className="px-3 py-2 text-left text-slate-300">Comment</th>
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100">
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12 space-y-6">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* ✅ Response rate + NPS dial side-by-side */}
          <div className="flex items-center gap-4">
            <ResponseDial value={responseRate} />
            <NpsDial value={nps} />
            <button
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 sm:p-6 shadow-xl shadow-black/40">
          <div className="mb-4">
            ...
            <p className="text-xs text-slate-400">
              Scores are shown when a response exists in /npsme/live/responses.csv.
            </p>
          </div>
                  ...
                  {completed.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.invitationId} className={stripe}>
                        ...
                        <td className="px-3 py-2 text-slate-300">
                          {resp?.comment ? String(resp.comment).slice(0, 140) : (
                            <span className="text-slate-500">-</span>
                          )}
                        </td>
                        <td className="px-3 py-2 text-slate-300">
