...
// --- Demo API (in-memory placeholder; swap to Dropbox later) ---
let demoResponses = []; // {score, comment, ts}
...
app.get("/api/demo/metrics", (_req, res) => {
  ...
  const passives  = demoResponses.filter(r => r.score >= 7 && r.score <= 8).length;
...
app.post("/api/intake", (req, res) => {
  // For now, just log. Later: save to Dropbox.
